...
export default async function IcpEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await supabase.from('icps').select('id')
  ...
  if (error) {
    return (
      <div className="container-px mx-auto max-w-4xl py-8">
        ...
        <p className="mt-4"><a className="underline text-[--primary]" href="/icps/inventory">Back to All ICPs</a></p>
      </div>
    )
  }
