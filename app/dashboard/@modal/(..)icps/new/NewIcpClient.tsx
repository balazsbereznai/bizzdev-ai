...
export default function NewIcpClient() {
  ...
  const createViaApi = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const res = await fetch('/api/icps/new', { method: 'POST', cache: 'no-store' });
      if (!res.ok) {
  ...
  if (busy) {
    return (
      <div className="p-6">
        ...
        <div className="text-sm text-[--color-ink-3]">Creating a new Potential Customer…</div>
        <style jsx>{`
          .indeterminate-bar {
