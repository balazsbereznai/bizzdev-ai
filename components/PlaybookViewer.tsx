  ...
  return (
    <div className={["prose prose-sm md:prose-base lg:prose-lg max-w-none", className].join(' ')}>
      <ReactMarkdown
        ...
        components={{
          /* Headings: gentle control; Typography handles most */
          ...
          /* Tables: responsive wrapper + soft borders (Typography supplies base styles) */
          table: ({node, ...props}) => (
          ...
          th: ({node, ...props}) => <th className="text-left font-medium px-3 py-2 align-top" {...props} />,
          td: ({node, ...props}) => <td className="px-3 py-2 align-top" {...props} />,

          /* Blockquote */
          blockquote: ({node, ...props}) => (
            <blockquote className="border-l-4 border-[--border] pl-4 italic text-black/70" {...props} />
          ),
